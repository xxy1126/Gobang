...
MainSceneOnline::MainSceneOnline(QWidget *parent) : QMainWindow(parent)
{
    //鼠标追踪事件
   ...
   repentanceBtn->move(this->width()-repentanceBtn->width()*1.2,this->height()*0.5);
    ...
    connectLabel->setFont(QFont("华文行楷",15));
    connectLabel->move(this->width()-giveupBtn->width()*1.3,this->height()*0.15);
    initGame();

}

void MainSceneOnline::paintEvent(QPaintEvent *)
{
...
void MainSceneOnline::mouseMoveEvent(QMouseEvent *event)
{
    //通过鼠标的hover确定落子的标记
    int x = event->x();
    int y = event->y();
...
void MainSceneOnline::mouseReleaseEvent(QMouseEvent *)
{
    if(connectSuccess == false)
    {
        QMessageBox::information(this, "hi", "未连接，请等待");
        return ;
    }
    if(selectPos == false)
        return;
...
void MainSceneOnline::chessOneByOther()
{
...
void MainSceneOnline::chessOneByPerson()
{
    //根据当前存储的坐标下子
    //只有有效点击,并且该处没有子才下子
    if(clickPosRow != -1 && clickPosCol != -1 && game->gameMapVec[clickPosRow][clickPosCol] == 0)
...
void MainSceneOnline::ifEndGame()
{
...
void MainSceneOnline::repentance()
{
...
void MainSceneOnline::repentanceByEnemy()
{
...
void MainSceneOnline::yesRegret()
{
...
void MainSceneOnline::noRegret()
{
    QMessageBox::information(this, "hi", "对方不同意悔棋");
}
void MainSceneOnline::giveup()
{
    if(connectSuccess == false)
    {
        return ;
...
void MainSceneOnline::giveUpByEnemy()
{
    game->gamestatus = OVER;
